...
file_path <- "/data/nehe/stinglessbee/ticks/t_data"
head(list.files(file_path))

#sorting the data
dataF <- sort(list.files(file_path, pattern="_1.fastq", full.names = TRUE))
head(dataF)
dataR <- sort(list.files(file_path, pattern="_2.fastq", full.names = TRUE))
head(dataR)
...
tick_sample_metadata <- read.csv("tick_sample_metadata.csv", sep = ",", header = T)
tick_sample_metadata <- tick_sample_metadata[,-1]
sdata <- tick_sample_metadata %>% remove_rownames %>% column_to_rownames(var="Sample_ID")
...
biomformat::write_biom(biomformat::make_biom(data = t(as.matrix(otu_table(physeq)))), 
                       "/data/nehe/stinglessbee/tick.biom")
...
M_blast_abundance <- silva_blast_raw[,c(1, 9:66)]
...
Featured_table <- merged_data[,c(7,15:72)]
...
group <- Featured_table %>%
  group_by(Genus)%>%
  summarise_each(funs(sum), "KW188","KW200","KW330","KW400","KW527","KW566","KW573","KW700","KW740","KW748","KL920","KL940","KL950","KL1026","KL1087","KW220","KW285","KW487","KW489","KW610","KW650","KW805","KW806",
                 "KW845","KW850","KL955","KL1101","KL1102","KL1103","KW64","KW98","KW240","KW323","KW345","KW395","KW578","KW657","KW711","KW821","KL933","KL944","KL1091","KL1082","KL1100","KW211","KW212",
                 "KW213","KW214","KW215","KW216","KW217","KW218","KW222","KW490","KW651","KW847","KL956","KL1059")
...
male_infected <- group[,c(1,2:16)]
female_infected <- group[,c(1,17:30)]
male_uninfected <- group[,c(1,31:45)]
female_uninfected <- group[,c(1,46:59)]


male_infected_total <- male_infected %>% adorn_totals(c("col"))
male_infected_total <- mutate(male_infected_total, male_infected=rowSums(male_infected_total[17])/15)
male_infected_total <- male_infected_total[,c(1,18)]

female_infected_total <- female_infected %>% adorn_totals(c("col"))
female_infected_total <- mutate(female_infected_total, female_infected=rowSums(female_infected_total[16])/14)
female_infected_total <- female_infected_total[,c(1,17)]

male_uninfected_total <- male_uninfected %>% adorn_totals(c("col"))
male_uninfected_total <- mutate(male_uninfected_total, male_uninfected=rowSums(male_uninfected_total[17])/15)
male_uninfected_total <- male_uninfected_total[,c(1,18)]

female_uninfected_total <- female_uninfected %>% adorn_totals(c("col"))
female_uninfected_total <- mutate(female_uninfected_total, female_uninfected=rowSums(female_uninfected_total[16])/14)
female_uninfected_total <- female_uninfected_total[,c(1,17)]


#merging the above dataframes
merged <- Reduce(function(x,y) merge(x,y,by="Genus",all=TRUE), list(male_infected_total,female_infected_total,male_uninfected_total,female_uninfected_total))
...
to_represent <- c("Rickettsia", "Coxiella", "Staphylococcus", "Trueperella", "Pseudomonas", "Corynebacterium","Brevundimonas", "Lyticum", "Micrococcus", "Streptomyces", "Serratia","Burkholderia", "Acinetobacter", "Peptoniphilus", "Others")
...
bar$Genus <- factor(bar$Genus, levels=c("Rickettsia", "Coxiella", "Staphylococcus", "Trueperella", "Pseudomonas", "Corynebacterium","Brevundimonas", "Lyticum", "Micrococcus", "Streptomyces", "Serratia","Burkholderia", "Acinetobacter", "Peptoniphilus", "Others"))

# Choosing the colours to use in the barplot
myPalette <- c("#89C5DA", "#DA5724", "#74D944", "#CE50CA", "#3F4921", "#C0717C", "#CBD588", "#5F7FC7", "#673770", "#D3D93E", "#38333E", "#508578", "#D7C1B1", "#689030", "#AD6F3B")
...
lbs[match("Rickettsia", brk)] = expression(italic("Rickettsia"))
lbs[match("Coxiella", brk)] = expression(italic("Coxiella"))
lbs[match("Staphylococcus", brk)] = expression(italic("Staphylococcus"))
lbs[match("Trueperella", brk)] = expression(italic("Trueperella"))
lbs[match("Pseudomonas", brk)] = expression(italic("Pseudomonas"))
lbs[match("Corynebacterium", brk)] = expression(italic("Corynebacterium"))
lbs[match("Brevundimonas", brk)] = expression(italic("Brevundimonas"))
lbs[match("Lyticum", brk)] = expression(italic("Lyticum"))
lbs[match("Micrococcus)", brk)] = expression(italic("Micrococcus"))
lbs[match("Streptomyces", brk)] = expression(italic("Streptomyces"))
lbs[match("Serratia", brk)] = expression(italic("Serratia"))
lbs[match("Burkholderiar", brk)] = expression(italic("Burkholderia"))
lbs[match("Acinetobacter", brk)] = expression(italic("Acinetobacter"))
lbs[match("Peptoniphilus", brk)] = expression(italic("Peptoniphilus"))
...
p <- ggplot(bar,aes(x = fct_inorder(Infection_status), y = abundance), labs(fill= Genus), group=row.names(bar))+ xlab("Infection_status")+ ylab("abundance") + geom_col(aes(fill = Genus),position = position_stack(reverse = FALSE))+
...
p + scale_y_continuous(labels = scales::percent_format(accuracy = 1)) + xlab("Infection_status") + theme(strip.background = element_rect(colour = "black", fill = "white")) + theme(strip.text.x = element_text(colour = "white", face = "bold")) + theme(panel.spacing = unit(1, "lines"))



#plotting the barplot #trial #coord_flip
p <- ggplot(bar_210523,aes(x = fct_inorder(sample_names), y = abundance), labs(fill= Genus), group=row.names(bar_210523))+ xlab("Species")+ ylab("Abundance") + geom_col(aes(fill = Genus),position = position_stack(reverse = FALSE))+
...
M.bocandei<- M.bocandei_total[!(M.bocandei_total$M.bocandei == 0),]
M.togoensis<- M.togoensis_total[!(M.togoensis_total$M.togoensis == 0),]
M.ferruginea<- M.ferruginea_total[!(M.ferruginea_total$M.ferruginea == 0),]
M.lendliana<- M.lendliana_total[!(M.lendliana_total$M.lendliana == 0),]
D.schmidti<- D.schmidti_total[!(D.schmidti_total$D.schmidti == 0),]
Hypotrigona_spp_1<- Hypotrigona_spp_1_total[!(Hypotrigona_spp_1_total$Hypotrigona_spp_1 == 0),]
Hypotrigona_spp_2<- Hypotrigona_spp_2_total[!(Hypotrigona_spp_2_total$Hypotrigona_spp_2 == 0),]
Liotrigona_spp<- Liotrigona_spp_total[!(Liotrigona_spp_total$Liotrigona_spp == 0),]


M.bocandei <- M.bocandei %>% select(Genus)
M.togoensis<- M.togoensis %>% select(Genus)
M.ferruginea<- M.ferruginea %>% select(Genus)
M.lendliana <- M.lendliana %>% select(Genus)
D.schmidti<- D.schmidti %>% select(Genus)
Hypotrigona_spp_1 <- Hypotrigona_spp_1 %>% select(Genus)
Hypotrigona_spp_2 <- Hypotrigona_spp_2 %>% select(Genus)
Liotrigona_spp <- Liotrigona_spp %>% select(Genus)
...
####Permanova for different hive - nehe draft
physeq6 <- physeq3
physeq6
...
##-----------
hdata <- read.csv("99_data.csv", sep = ",", header = T)
hdata <- hdata[,-1]
View(hdata)
attach(hdata)

#creating multiple dataframes for the different treatments
female_uninfected <- hdata[,c(1,2:15)]
female_infected <- hdata[,c(1,16:29)]
male_uninfected <- hdata[,c(1,30:44)]
male_infected <- hdata[,c(1,45:59)]

female_uninfected_total <- female_uninfected %>% adorn_totals(c("col"))
#female_uninfected_total <- mutate(female_uninfected_total, female_uninfected=rowSums(female_uninfected_total[16])/14)
female_uninfected_total <- female_uninfected_total[,c(1,16)]
colnames(female_uninfected_total) <- c('Genus','female_uninfected_total')

female_infected_total <- female_infected %>% adorn_totals(c("col"))
#female_infected_total <- mutate(female_infected_total, female_infected=rowSums(female_infected_total[16])/14)
female_infected_total <- female_infected_total[,c(1,16)]
colnames(female_infected_total) <- c('Genus','female_infected_total')

male_uninfected_total <- male_uninfected %>% adorn_totals(c("col"))
#male_uninfected_total <- mutate(male_uninfected_total, male_uninfected=rowSums(male_uninfected_total[17])/15)
male_uninfected_total <- male_uninfected_total[,c(1,17)]
colnames(male_uninfected_total) <- c('Genus','male_uninfected_total')


male_infected_total <- male_infected %>% adorn_totals(c("col"))
#male_infected_total <- mutate(male_infected_total, male_infected=rowSums(male_infected_total[17])/15)
male_infected_total <- male_infected_total[,c(1,17)]
colnames(male_infected_total) <- c('Genus','male_infected_total')



#merging the above dataframes
merged <- Reduce(function(x,y) merge(x,y,by="Genus",all=FALSE),list(female_uninfected_total,female_infected_total,male_uninfected_total,male_infected_total))
